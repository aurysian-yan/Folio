...
## 39. P3C macOS 字体导入与操作

库根记录区分 `directory` 与 `file`。旧目录记录在 schema v3 迁移后默认为
`directory`；单文件根只扫描选定文件，文件暂时不可用时保留其缓存。
UniFFI 为每个字款返回全部实际来源文件及其所属根，macOS 操作始终使用用户
选定的文件路径。同字族的其他来源不会随之注册。

macOS 导入可复制至应用数据目录或保存安全作用域书签并引用原文件。默认复制且
不询问；用户可在设置中更改默认方式或启用逐次询问。Folio 记录副本、引用、
会话激活和持久安装；会话激活使用 Core Text `.session`，安装副本使用
`.persistent`。卸载撤销持久注册并清理仅为安装建立的副本。只有 Folio 管理的
导入副本可从检查器移到废纸篓，引用文件和用户目录原文件不会被删除。

导入与操作按文件返回结果，失败项可重试。文件监听、热重载及 WebDAV 不属于
P3C。
