...
Folio 是一款跨设备字体资产管理工具。用户可以从本地目录或单个字体文件建立字体库，查找、预览和整理字体，并在支持的平台上将字体临时启用或持久安装。未来还将支持 WebDAV/123PAN 同步、离线访问、移动端文件提供与导出，以及基于查询条件动态匹配字体的智慧收藏夹。
...
WebDAV 是共享云同步协议，123PAN 作为 WebDAV 服务接入，不另建一套同步架构。iCloud/CloudKit 暂不纳入：CloudKit 需要 Apple Developer Program 会员；iCloud Drive 文件夹来源也不作为替代方案。

### 手动收藏夹与智慧收藏夹

手动收藏夹由用户明确管理成员；后续规划的智慧收藏夹保存搜索与筛选条件，并根据当前字体库动态展示匹配结果。两者在界面和数据语义上保持区分，智慧收藏夹复用共享查询能力，不另写一套字体匹配规则。
...
- 智慧收藏夹持久化查询条件，匹配成员按当前字体库动态计算；手动收藏夹继续保存用户指定的成员。
...
| 阶段 3B：macOS 字体库 UI | 基础界面已完成；封面流待收尾 | SwiftUI 浏览模式与现有大卡片轮播 |
...
| iCloud/CloudKit | 明确排除 | CloudKit 需要付费开发者会员；不采用 iCloud Drive 文件夹替代方案 |

macOS 收藏夹名称、图标与配色编辑已实现；其他平台仍需各自实现对应界面。
...
#### 3B：字体库浏览界面与封面流（基础界面已完成，封面流待收尾）

维护原生侧栏、工具栏、搜索、Facet、浏览模式、字体预览、Inspector 与底部预览栏。收藏夹名称、图标和配色可在创建后编辑，更新时保持原 `CollectionId` 和成员关系。

增加独立的“封面流”浏览模式，作为现有四种视图之外的第五种模式，不替换现有“展开卡片”。

- 以用户提供的 iPod 横屏截图为视觉参考：当前字体卡片正面突出，两侧卡片缩小并作透视倾斜，卡片下方显示渐隐倒影。
- 复用现有大卡片轮播的字体选择、分页、懒加载与真实预览，不改 Rust 查询语义；调整工具栏视图选择器以容纳第五种模式。
- 使用 SwiftUI 展示能力和语义化主题色，适配浅色、深色及 `prefers-reduced-motion`；保持键盘选择、VoiceOver 标签和当前所选字体的可见状态。

**完成门槛：**封面流可由工具栏和设置独立选择；中心卡片与两侧卡片在滚动时平滑切换；既有视图模式、选中字体、分页和预览行为保持正确；浅色、深色、减弱动态效果与键盘/读屏操作均可用。

#### 3C：macOS 字体操作与批量导入（已实现）

macOS 平台适配使用 Core Text 会话注册实现字体启用/停用，使用持久注册实现安装/卸载；Core 仍只提供平台无关的数据。导入支持复制到 Folio 或引用原文件，并对多文件操作逐项报告结果和允许重试。Finder 多选文件可批量交给 Folio，导入结果可继续批量启用或安装。

**完成依据：**实现位于 macOS `FontOperations`、`LibraryViewModel` 与导入结果界面；源码包含会话/持久字体注册、逐文件结果、失败重试和批量操作。

#### 3D：macOS 增量刷新（基础流程已实现）

应用启动、添加字体目录、导入字体及托管副本移除后均会触发目录增量刷新，并更新可见查询与操作状态。

文件系统目录监听、热重载和版本变化呈现延期为后续增强，不阻塞 WebDAV、Android 或字符表阶段。

**阶段 3 完成门槛：**macOS 本地浏览、收藏夹管理、字体启用/安装、批量导入和增量刷新均基于真实数据且错误可恢复；文件系统监听与热重载不是阶段完成条件。

### 阶段 4：共享云同步基础（为 P1 Android 提供依赖）

在本地身份、修订、收藏夹与冲突基础稳定后，实现共享 WebDAV 同步，为 Android 的主要使用场景提供支持，也允许其他平台复用。123PAN 通过 WebDAV 服务配置接入，不作为独立同步协议。
...
- 不实现 CloudKit 或其他 iCloud 接入，也不将 iCloud Drive 文件夹列为同步来源。
...
在 WebDAV 和 Android 阶段之后实现字符表共享能力，为 macOS 与 Windows 桌面提供完整页面；Android/iOS 可在各自平台阶段复用同一映射接口适配屏幕。

共享能力与行为：

- Rust 从选中 Face 的 `cmap` 枚举 Unicode 标量、Glyph ID 和共享文字系统分类，按码点稳定排序并分页；根据真实来源与 TTC/OTC face index 定位正确成员。
- UniFFI 与 Tauri 暴露同一数据、错误和搜索/过滤语义；前端不得自行解析 cmap 或推断文字系统。
- 支持按字符或 `U+` 码点查找，并按接口提供的文字系统筛选。第一版不包含未编码 glyph、连字塑形或多码点序列浏览。
- 从字体详情入口进入独立字符表页面；返回时保留原字体库查询、筛选与选中项。页面包含字体/字样式上下文、搜索、文字系统筛选、按需加载的自适应网格；选中后显示较大字形、码点、Glyph ID，并可复制字符和 Unicode 值。
- 为加载、空映射、无匹配、来源离线/不可读和损坏文件提供真实且可恢复的状态。

平台验证：

- macOS 使用 CoreText 或经过验证的本地字体渲染方式，准确对应来源与 collection face index，不要求先安装字体。
- Windows WebView2 验证本地字体、非零 collection face index 和可变轴的准确 Face 呈现；不能保证选中 Face 时，不得静默回退到系统相似字体。
- Linux 的 React 页面和数据契约从此阶段纳入设计；真实 WebKitGTK 验证在阶段 9 完成，不能由其他平台构建替代。
- 各平台栅格像素可以不同，但字符映射、Face、轴值和回退行为一致。

**完成门槛：**共享映射与搜索/筛选一致；macOS 和 Windows 验证实际来源及准确 Face 呈现；分页、大型 cmap、变量字体、collection member、键盘、读屏与明暗外观状态均有可复核结果。Linux 运行时验证作为阶段 9 的完成条件。

### 阶段 7：P2 Windows 桌面客户端
...
### 阶段 9：P4 Linux 与 WOFF/WOFF2（v2）

Linux 桌面与 WOFF/WOFF2 均按 v2 规划同期推进。Linux 复用 Windows 的 Tauri/React 页面，增加 Linux 字体服务、打包、权限、桌面环境和 WebKitGTK 适配；阶段 6 的字符表必须在 Linux 真实环境验证。
...
在现有平台主线完成后，提供类似 Steam 智慧收藏夹的动态字体分组。用户可保存搜索与筛选条件；字体库或条件变化后，收藏夹自动显示当前匹配的字体。它与手动收藏夹并列存在，不改写手动成员关系。

- 复用 `folio-query` 已定义的搜索、范围、Facet 和排序语义；条件组合遵循统一查询规则。
- 保存、编辑和删除智慧收藏夹及其查询条件；匹配结果由当前可用字体数据动态计算，不将结果固化为手动成员列表。
- 在已支持的平台呈现一致的智慧收藏夹与匹配行为，并纳入既有用户状态持久化及同步设计。
- 提供清晰的空结果与条件编辑体验；键盘、读屏、浅色和深色模式均可用。

**完成门槛：**同一组已保存条件在各平台产生一致的匹配结果；字体库刷新、条件编辑及离线/同步状态变化后，结果正确更新；手动收藏夹成员保持独立且不受智慧收藏夹重算影响。
...
1. 完成 macOS SwiftUI 收尾，新增封面流第五种视图模式。
2. 建立 WebDAV/123PAN 共享同步基础。
3. 推进 P1 Android，完成同步、Provider/Picker 与 SAF/USB 场景。
4. 推进跨平台字符表，完成共享映射契约、macOS 页面和 Windows WebView2 准确 Face 验证。
5. 推进 P2 Windows，复用字符表、共享 Rust 能力及 Tauri/React 基线。
6. 推进 P3 iOS/iPadOS。
7. 在 v2 推进 P4 Linux，并同期完成 WOFF/WOFF2 管理与预览；完成字符表 WebKitGTK 真实环境验证。

macOS 文件系统监听与热重载作为后续增强，不阻塞以上顺序；iCloud/CloudKit 完全排除。

阶段 10 智慧收藏夹在上述当前主线完成后推进。
